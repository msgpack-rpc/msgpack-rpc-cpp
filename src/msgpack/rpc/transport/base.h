...
#define MSGPACK_RPC_DGRAM_BUFFER_SIZE (64*1024)
            ...
            inline bool default_reference_func(type::object_type /*type*/, std::size_t /*len*/, void*) { return true; }
            
            template <typename MixIn>
            inline stream_handler<MixIn>::stream_handler(int fd, loop lo) :
            mp::wavy::handler(fd),
            m_pac(default_reference_func, nullptr, MSGPACK_RPC_STREAM_BUFFER_SIZE),
            m_loop(lo) { }
